...
- 单击系统托盘图标可重新显示并聚焦主窗口。
- Windows/Linux 使用 36px 单行标题栏，将应用图标、`DSH Desktop <版本号>`、应用/编辑/更新/帮助菜单、更新状态和三个窗口按钮合并展示；浅色与深色配色跟随系统主题，加载 DSH 页面后仍保持可见。
- macOS 保留系统原生标题栏和系统菜单栏；各平台菜单与托盘共用同一状态和生命周期。
- “帮助 > 关于 DSH Desktop”展示构建时间、构建标识、平台架构和内置 DSH/Node/pnpm 版本，并可复制公开版本信息。
- 托盘或标题栏菜单提供运行状态、更新状态、DSH 终端、重新启动 Harness、检查更新和明确退出。
- 只有托盘或标题栏菜单的明确“退出”操作才会回收桌面壳、Node 和完整 DSH 进程树。
...
- 应用启动约 30 秒后检查更新，长期驻留托盘时每 24 小时复查一次；托盘和顶部菜单也可以随时手动检查。
- 发现新版本后使用固定尺寸窗口展示版本更新内容，长内容只滚动正文；完整安装包验签成功后才停止 Sidecar。
- 更新包保存在应用私有缓存目录；网络中断、45 秒无进度或应用重启后从严格校验的 HTTP Range 偏移继续下载。
- 标题栏和托盘显示百分比、已下载/总大小、平均速度、预计剩余时间及续传次数；失败后可从帮助菜单复制脱敏诊断。
